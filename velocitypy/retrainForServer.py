...
def create_inception_graph():
  ...
  # with tf.Session() as sess:
   # new_saver = tf.train.import_meta_graph('exportmodel/00000001/export.meta')
   # new_saver.restore(sess, 'exportmodel/00000001/export')
   # model_filename = os.path.join(
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope('cross_entropy'):
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=
      logits, labels=ground_truth_input)
...
def train(retrain):
  ...
  sess = tf.Session()
  if retrain:
      saver = tf.train.import_meta_graph(FLAGS.model_dir + '/my-model-forretraining.meta')
  ...
  # Merge all the summaries and write them out to retrain_logs (by default)
  ...
  if not retrain:
      ...
      for i in range(FLAGS.how_many_training_steps):
        ...
        # saver.save(sess,FLAGS.model_dir+ '/my-model-forretraining')
  ...
  if retrain:
      with gfile.FastGFile(os.path.join(
              FLAGS.model_dir, FLAGS.output_labels), 'w') as f:
          f.write('\n'.join(image_lists.keys()) + '\n')
      saver.save(sess, FLAGS.model_dir + '/my-model-forretraining')
      output_graph_def = graph_util.convert_variables_to_constants(
          sess, graph.as_graph_def(), [FLAGS.final_tensor_name])
      with gfile.FastGFile(os.path.join(
              FLAGS.model_dir, FLAGS.output_graph), 'wb') as f:
  ...
  sess.close()
  return os.path.join(FLAGS.model_dir, FLAGS.output_graph), os.path.join(FLAGS.model_dir, FLAGS.output_labels)
# TEST EXPORT MODEL
...
#          #scores_tensor=values),
  ...
  #     'inputs': exporter.generic_signature({'images': jpeg_data_tensor}),
  #     'outputs': exporter.generic_signature(
  #       {
  #         'classes': image_lists.keys(),
  #         'scores': bottleneck_input
  #
  #       })}
  #     )
  # model_exporter.export(export_path, tf.constant(0001), sess)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--image_dir',
      type=str,
      default='/Volumes/LaCie/viega/annotations',
  ...
  parser.add_argument(
      '--model_dir',
      type=str,
      default='/Volumes/LaCie/viega/imagenet',
      help="""\
      Path to classify_image_graph_def.pb,
      imagenet_synset_to_human_label_map.txt, and
      imagenet_2012_challenge_label_map_proto.pbtxt.\
  ...
  parser.add_argument(
      '--bottleneck_dir',
      type=str,
      default='/Volumes/LaCie/viega/bottleneck',
