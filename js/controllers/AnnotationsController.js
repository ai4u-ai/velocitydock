...
*/
MetronicApp.controller('AnnotationsController',function ($scope, API_ENDPOINT,AuthService,$http,$location, rx,$location, $anchorScroll) {
        ...
        $http.get('/api/anotations/loadAll').success(function (data) {
           // console.log(data);
            ...
            $scope.files= data.map(function(obj){
                ...
                return {_id:obj._id,uploadedBy:obj.uploadedBy,annotation:obj.annotation,mediaFile:{_id:obj.annotatedOnMedia.media._id,src:API_ENDPOINT.url+'/stream/video/'+obj.annotatedOnMedia.media.id+'?'+'access_token='+AuthService.getAuthToken().substring(4,AuthService.getAuthToken().length),type: "video/mp4"}}
